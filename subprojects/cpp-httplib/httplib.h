...
#include <pistache/eventmeth.h> // for PS_LOG_DEBUG...
...
namespace httplib
{
    ...
    using ContentReceiverWithProgress = std::function<bool(const char* data, size_t data_length, uint64_t offset,
    ...
    namespace detail
    {
        ...
        {
            PS_LOG_DEBUG_ARGS("::close actual_fd %d", sock);
#ifdef _WIN32
        ...
        inline bool wait_until_socket_is_ready(socket_t sock, time_t sec, time_t usec)
        {
            ...
            if (ret > 0 && (FD_ISSET(sock, &fdsr) || FD_ISSET(sock, &fdsw)))
            {
                int error          = 0;
                socklen_t len      = sizeof(error);
                int getsockopt_res = getsockopt(sock, SOL_SOCKET, SO_ERROR,
                                                reinterpret_cast<char*>(&error), &len);

                bool res = ((getsockopt_res >= 0) && !error);
#ifdef DEBUG
                if (!res)
                    PS_LOG_DEBUG_ARGS("Sock fail, getsockopt_res %d, socket error %d",
                                      getsockopt_res, error);
#endif
...
#endif
                PS_LOG_DEBUG_ARGS("::socket actual_fd %d", sock);
...
#define USE_IF2IP
            ...
            time_t write_timeout_usec, const std::string& intf, Error& error)
        {
            auto sock = create_socket(
                ...
                [&](socket_t sock2, struct addrinfo& ai) -> bool {
                    ...
                    if (ret < 0)
                    {
                        if (is_connection_error() || !wait_until_socket_is_ready(sock2, connection_timeout_sec, connection_timeout_usec))
                        {
        ...
        {
            return (!content_type.find("text/") && content_type != "text/event-stream") || content_type == "image/svg+xml" || content_type == "application/javascript" || content_type == "application/json" || content_type == "application/xml" || content_type == "application/xhtml+xml";
        }

        enum class EncodingType { None = 0,
                                  Gzip,
                                  Brotli };
        ...
        {
            if (req.method == "POST" || req.method == "PUT" || req.method == "PATCH" || req.method == "PRI" || req.method == "DELETE")
            {
                return true;
            }
            ...
            const std::string& password, bool is_proxy = false)
        {
            ...
            auto field = "Digest username=\"" + username + "\", realm=\"" + auth.at("realm") + "\", nonce=\"" + auth.at("nonce") + "\", uri=\"" + req.path + "\", algorithm=" + algo + ", qop=" + qop + ", nc=\"" + nc + "\", cnonce=\"" + cnonce + "\", response=\"" + response + "\"";
 ...
 */
                            ...
                            U SSL_connect_or_accept, V setup)
        {
            ...
            if (ssl)
            {
                ...
                if (!setup(ssl) || SSL_connect_or_accept(ssl) != 1)
                {
                    ...
                    PS_LOG_DEBUG("setup(ssl) or SSL_connect_or_accept(ssl) failed");
            ...
            PS_LOG_DEBUG_ARGS("Returning ssl %p", ssl);
            return ssl;
        }
                                               ...
                                               time_t timeout_usec)
        {
            ...
            while ((res = ssl_connect_or_accept(ssl)) != 1)
            {
                ...
                case SSL_ERROR_WANT_READ:
                    PS_LOG_DEBUG("SSL_ERROR_WANT_READ");
                    if (select_read(sock, timeout_sec, timeout_usec) > 0)
                    {
                        PS_LOG_DEBUG("select_read succeeded");
                        continue;
                    }
                    PS_LOG_DEBUG("select_read zero fds");
                    break;
                case SSL_ERROR_WANT_WRITE:
                    PS_LOG_DEBUG("SSL_ERROR_WANT_WRITE");
                    if (select_write(sock, timeout_sec, timeout_usec) > 0)
                    {
                        PS_LOG_DEBUG("select_write succeeded");
                        continue;
                    }
                    PS_LOG_DEBUG("select_write zero fds");
                    break;
                default:
                    PS_LOG_DEBUG("Other err code %d", (int)err)
                    break;
                }
                PS_LOG_DEBUG("Returning false");
                return false;
            }
            PS_LOG_DEBUG("Returning true");
            return true;
        }
...
#ifdef _WIN32
                    while (err == SSL_ERROR_WANT_READ || err == SSL_ERROR_SYSCALL && WSAGetLastError() == WSAETIMEDOUT)
                    {
#else
                                ...
                                const char* client_ca_cert_dir_path)
    {
        ...
        if (ctx_)
        {
            ...
            if (SSL_CTX_use_certificate_chain_file(ctx_, cert_path) != 1 || SSL_CTX_use_PrivateKey_file(ctx_, private_key_path, SSL_FILETYPE_PEM) != 1)
            {
                SSL_CTX_free(ctx_);
                ctx_ = nullptr;
            }
    ...
    inline bool SSLServer::process_and_close_socket(socket_t sock)
    {
        ...
        auto ssl = detail::ssl_new(
            ...
            [&](SSL* ssl) {
                PS_LOG_DEBUG("Calling ssl_connect_or_accept_nonblocking");
        ...
        if (ssl)
        {
            PS_LOG_DEBUG("Calling process_server_socket_ssl");
            ...
            PS_LOG_DEBUG("Calling ssl_delete");

            const bool shutdown_gracefully = ret;
            detail::ssl_delete(ctx_mutex_, ssl, shutdown_gracefully);
        }

        PS_LOG_DEBUG("Shutdown and close socket");
        detail::shutdown_socket(sock);
        detail::close_socket(sock);
        return ret;
    }
    ...
    inline bool SSLClient::initialize_ssl(Socket& socket, Error& error)
    {
        ...
        auto ssl = detail::ssl_new(
            ...
            [&](SSL* ssl) {
                ...
                {
                    PS_LOG_DEBUG("server certificate verification");

                    if (!load_certs())
                    {
                        error = Error::SSLLoadingCerts;
                        PS_LOG_DEBUG("server certificate verification failed");
                        return false;
                    }
                ...
                {
                    PS_LOG_DEBUG("ssl_connect_or_accept_nonblocking failed");
                ...
                if (server_certificate_verification_)
                {
                    ...
                    {
                        PS_LOG_DEBUG("SSL_get_peer_certificate failed");
                        error = Error::SSLServerVerification;
                        return false;
                    }
                    ...
                    {
                        PS_LOG_DEBUG("verify_host(server_cert) failed");
        ...
        PS_LOG_DEBUG("Shutting down and closing socket");
