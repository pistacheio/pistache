...
TEST(headers_test, accept) {
...

  {
    ...
    ASSERT_TRUE(mime.q().getOrElse(Pistache::Http::Mime::Q(0)) ==
                Pistache::Http::Mime::Q(20));
  }
...

  {
    ...
    ASSERT_TRUE(media[0].q().getOrElse(Pistache::Http::Mime::Q(0)) ==
                Pistache::Http::Mime::Q(30));
    ...
    ASSERT_TRUE(media[4].q().getOrElse(Pistache::Http::Mime::Q(0)) ==
                Pistache::Http::Mime::Q::fromFloat(0.5));
  }
...
TEST(headers_test, authorization_bearer_test) {
  ...
  ASSERT_TRUE(
      ...
      "d0131JxqX4xSZLlO5xMRrCPBgn_00OxKJ9CQdnpjpuzblNQd2-A" == oss.str());
  ASSERT_TRUE(
      au.value() ==
      "Bearer "
      "eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXUyJ9."
...
TEST(headers_test, date_test_rfc_1123) {
  ...
  Pistache::Http::FullDate::time_point expected_time_point =
      date::sys_days(date::year{1994} / 11 / 6) + hours(8) + minutes(49) +
      seconds(37);
...
TEST(headers_test, access_control_allow_headers_test) {
  ...
  ASSERT_TRUE(std::strcmp(os.str().c_str(),
                          ...
                          "Authorization, X-Requested-With") == 0);
  ASSERT_TRUE(allowHeaders.val() ==
...
TEST(headers_test, add_new_header_test) {
  ...
  const auto &headersList =
      Pistache::Http::Header::Registry::instance().headersList();
  const bool isFound = std::find(headersList.begin(), headersList.end(),
                                 headerName) != headersList.end();
  ASSERT_TRUE(isFound);
}
