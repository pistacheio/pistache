...
public:
  ...
  explicit ViewBase(std::nullptr_t) : begin_(nullptr), size_(0) {}

  explicit ViewBase(std::nullptr_t, size_t) : begin_(nullptr), size_(0) {}

  explicit ViewBase(const T *begin, size_t size) : begin_(begin), size_(size) {}

  explicit ViewBase(const T *begin, const T *end) : begin_(begin), size_(0) {
    if (begin > end) {
...
template <typename T> struct View : public ViewBase<T> {
  explicit View(std::nullptr_t) : ViewBase<T>(nullptr) {}

  explicit View(std::nullptr_t, size_t) : ViewBase<T>(nullptr, 0) {}

  explicit View(const T *begin, size_t size) : ViewBase<T>(begin, size) {}

  explicit View(const T *begin, const T *end) : ViewBase<T>(begin, end) {}
};

template <> struct View<std::string> : public ViewBase<char> {

  typedef ViewBase<char> Base;

  explicit View(std::nullptr_t) : Base(nullptr) {}

  explicit View(const char *begin, size_t end) : Base(begin, end) {}

  explicit View(const char *begin, const char *end) : Base(begin, end) {}
  ...
  template <size_t N> bool operator==(const char (&arr)[N]) const {
    return equals(arr, N);
  }
...
template <typename T, size_t N> struct ViewBuilder<std::array<T, N>> {
  typedef T Type;
...
template <typename T, size_t N> struct ViewBuilder<T (&)[N]> {
  typedef T Type;
...
template <typename T> struct ViewBuilder<std::vector<T>> {
  typedef T Type;
...
template <> struct ViewBuilder<std::string> {
