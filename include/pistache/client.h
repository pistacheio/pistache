...
struct Connection : public std::enable_shared_from_this<Connection> {
    ...
    struct RequestData {
        ...
        RequestData(Async::Resolver rdresolve, Async::Rejection rdreject,
                    const Http::Request& rdrequest, OnDone rdonDone)
            : resolve(std::move(rdresolve))
            , reject(std::move(rdreject))
            , request(rdrequest)
            , onDone(std::move(rdonDone))
        { }
  ...
  private:
    ...
    struct RequestEntry {
        RequestEntry(Async::Resolver reresolve, Async::Rejection rereject,
                     std::shared_ptr<TimerPool::Entry> retimer, OnDone reonDone)
            : resolve(std::move(reresolve))
            , reject(std::move(rereject))
            , timer(std::move(retimer))
            , onDone(std::move(reonDone))
        { }
