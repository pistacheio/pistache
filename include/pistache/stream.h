...
template <typename CharT = char> class RawStreamBuf : public StreamBuf<CharT> {
public:
...
public:
  ...
  template <size_t M> explicit ArrayStreamBuf(char (&arr)[M]) {
    bytes.clear();
...
public:
  ...
  struct Token {
    explicit Token(StreamCursor &_cursor)
        : cursor(_cursor), position(cursor.buf->position()),
  ...
  struct Revert {
    explicit Revert(StreamCursor &_cursor)
        : cursor(_cursor), eback(cursor.buf->begptr()),
