...
Query::Query(
    std::initializer_list<std::pair<const std::string, std::string>> query_params)
    : params(query_params)
{ }
...
const Header::Collection& Message::headers() const {
    return headers_;
}

Header::Collection& Message::headers() {
    return headers_;
}

const CookieJar& Message::cookies() const {
    return cookies_;
}

CookieJar& Message::cookies() {
    return cookies_;
}
...
const Uri::Query& Request::query() const {
    return query_;
}
...
Async::Promise<ssize_t> ResponseWriter::sendImpl(Code code, const char* data,
    ...
    if(contentEncoding_ == Http::Header::Encoding::Deflate) {
    ...
    if(contentEncoding_ == Http::Header::Encoding::Identity) {
...
const CookieJar& ResponseWriter::cookies() const {
    return response_.cookies();
}

CookieJar& ResponseWriter::cookies() {
    return response_.cookies();
}
...
Header::Collection& ResponseWriter::headers() {
    return response_.headers();
}

Timeout& ResponseWriter::timeout() {
    return timeout_;
}
...
DynamicStreamBuf* ResponseWriter::rdbuf() {
    return &buf_;
}
...
ResponseWriter ResponseWriter::clone() const {
...
#undef OUT
        ...
        [=](ssize_t rdata) {
            return Async::Promise<ssize_t>::resolved(rdata);
        },
...
//  header...
void ResponseWriter::setCompression([[maybe_unused]] const Pistache::Http::Header::Encoding _contentEncoding) {


#ifdef PISTACHE_USE_CONTENT_ENCODING_DEFLATE
    // Application requested deflate compression...
    if( _contentEncoding == Http::Header::Encoding::Deflate) {
...
#define OUT(...)                                         \
    ...
    auto setContentType = [&](const Mime::MediaType& mimeContentType) {
        auto& headers = writer.headers();
        auto ct       = headers.tryGet<Header::ContentType>();
        if (ct)
            ct->setMime(mimeContentType);
        else
            headers.add<Header::ContentType>(mimeContentType);
    };
...
bool Timeout::isArmed() const {
    return armed;
}

Timeout::Timeout(Tcp::Transport* transport_, Http::Version http_version, Handler* handler_,
...
void Handler::setMaxRequestSize(size_t value) {
    maxRequestSize_ = value;
}

size_t Handler::getMaxRequestSize() const {
    return maxRequestSize_;
}

void Handler::setMaxResponseSize(size_t value) {
    maxResponseSize_ = value;
}

size_t Handler::getMaxResponseSize() const {
