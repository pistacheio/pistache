...
class Reactor::Impl {
public:
  Impl(Reactor *reactor) : reactor_(reactor) {}
...
public:
  explicit SyncImpl(Reactor *reactor)
      : Reactor::Impl(reactor), handlers_(), shutdown_(), shutdownFd(),
        poller() {
...
private:
  ...
  struct HandlerList {
    ...
    template <typename Func> void forEachHandler(Func func) const {
      for (size_t i = 0; i < index_; ++i)
        func(handlers.at(i));
    }
...
#undef CALL_MEMBER_FN
  ...
  struct Worker {
    ...
    explicit Worker(Reactor *reactor, const std::string &threadsName)
        : thread(), sync(new SyncImpl(reactor)), threadsName_(threadsName) {}
...
Reactor::Key::Key() : data_(0) {}

Reactor::Key::Key(uint64_t data) : data_(data) {}
