...
AddressParser::AddressParser(const std::string& data) {
    ...
    if (end_pos != std::string::npos && hasColon_) {
        ...
        char* tmp_end_ptr;
        std::strtol(port_.c_str(), &tmp_end_ptr, 10);
        hasNumericPort_ = *tmp_end_ptr == '\0';
    }
}
...
bool AddressParser::hasNumericPort() const {
    return hasNumericPort_;
}
