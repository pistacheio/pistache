...
void CacheDirective::init(Directive directive, std::chrono::seconds delta) {
    ...
    } else if(directive == SMaxAge) {
        data.sMaxAge = delta.count();
    } else if(directive == MaxStale) {
        data.maxStale = delta.count();
    } else if(directive ==MinFresh) {
