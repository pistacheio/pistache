...
struct ConcretePeer : Peer {
    [[maybe_unused]] ConcretePeer(Fd fd, const Address& peer_addr, void* ssl)
        : Peer(fd, peer_addr, ssl)
    { }
};
} // namespace

Peer::Peer(Fd fd, const Address& peer_addr, void* ssl)
    : fd_(fd)
    , addr(peer_addr)
    , ssl_(ssl)
    , id_(getUniqueId())
...
void Peer::setIdle(bool bIdle) {
    isIdle_ = bIdle;
}
bool Peer::isIdle() const {
    return isIdle_;
}
...
void Peer::associateTransport(Transport* transport) {
