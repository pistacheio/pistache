...
Cookie::Cookie(std::string cookie_name, std::string cookie_value)
...
void Cookie::write(std::ostream& os) const {
    os << name << "=" << value;
    if (path.has_value()) {
        const std::string& p_value = *path;
        os << "; ";
        os << "Path=" << p_value;
    }
    if (domain.has_value()) {
        const std::string& d_value = *domain;
        os << "; ";
        os << "Domain=" << d_value;
    }
    if (maxAge.has_value()) {
        int mage_value = *maxAge;
        os << "; ";
        os << "Max-Age=" << mage_value;
    }
    if (expires.has_value()) {
        const FullDate& exp_value = *expires;
        os << "; ";
        os << "Expires=";
        exp_value.write(os);
    }
...
void CookieJar::removeAllCookies() {
    cookies.clear();
}
