...
CookieJar::add(const Cookie& cookie) {
    ...
    for(std::list<Cookie>::const_iterator it = cookies.begin(); it!= cookies.end(); ++it) { // traversing through the cookie list.
        if(it->name.compare(cookieName) == 0 && it->value.compare(cookieValue) == 0) {
            return; // do not add a cookie if exact cookie is present.
        }
    }
...
CookieJar::get(const std::string& name) const {
    ...
    for(std::list<Cookie>::const_iterator it = cookies.begin(); it!= cookies.end(); ++it) { // const_iterator  instead of iterator.
