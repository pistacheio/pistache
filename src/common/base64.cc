...
vector<byte>::size_type Base64Decoder::CalculateDecodedSize() const
{
    ...
    if(m_Base64EncodedString.size() < 4)
        throw runtime_error("Base64 encoded stream should always be at least four bytes.");
    ...
    if((m_Base64EncodedString.size() % 4) != 0)
        throw runtime_error("Base64 encoded stream length should always be evenly divisible by four.");
    ...
    const auto InputSize = distance(
        m_Base64EncodedString.begin(), EndIterator);
    ...
    switch(InputSize % 4)
    {
        case 2: return DecodedSize + 1;
        case 3: return DecodedSize + 2;
        default: return DecodedSize;
    }
}
...
const vector<byte> &Base64Decoder::Decode()
{
    ...
    for(string::size_type Index = 2; Index < DecodedSize; Index += 3)
    {
        ...
        m_DecodedData.at(OutputOffset + 0) = DecodeCharacter(m_Base64EncodedString.at(InputOffset + 0)) << 2 | DecodeCharacter(m_Base64EncodedString.at(InputOffset + 1)) >> 4;
        m_DecodedData.at(OutputOffset + 1) = DecodeCharacter(m_Base64EncodedString.at(InputOffset + 1)) << 4 | DecodeCharacter(m_Base64EncodedString.at(InputOffset + 2)) >> 2;
        m_DecodedData.at(OutputOffset + 2) = DecodeCharacter(m_Base64EncodedString.at(InputOffset + 2)) << 6 | DecodeCharacter(m_Base64EncodedString.at(InputOffset + 3));
...
inline byte Base64Decoder::DecodeCharacter(const unsigned char Character) const
{
...
const string &Base64Encoder::Encode() noexcept
{
    ...
    for(string::size_type Index = 0; Index < OctetTriplets; ++Index)
    {
        ...
        m_Base64EncodedString.at(OutputOffset + 0) = EncodeByte(
            m_InputBuffer.at(InputOffset + 0) >> 2);
        ...
        m_Base64EncodedString.at(OutputOffset + 3) = EncodeByte(
            m_InputBuffer.at(InputOffset + 2) & static_cast<byte>(0x3F));
    ...
    switch(m_InputBuffer.size() % 3)
    {
        ...
        case 1:
            ...
            m_Base64EncodedString.at(OutputOffset + 1) =
                EncodeByte((m_InputBuffer.at(InputOffset + 0) & static_cast<byte>(0x03)) << 4);
