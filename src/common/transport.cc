    ...
    if (bytes == -1) {
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        if (totalBytes > 0) {
          ...
          handler_->onInput(&vbuffer[0], totalBytes, peer);
        }
      } else {
        if (errno == ECONNRESET) {
...
void Transport::asyncWriteImpl(Fd fd, Transport::WriteEntry &entry,
                               WriteStatus status) {
  asyncWriteImpl(fd, entry.flags, entry.buffer, std::move(entry.deferred),
                 status);
}

void Transport::asyncWriteImpl(Fd fd, int flags, const BufferHolder &buffer,
  ...
  for (;;) {
    ...
    if (bytesWritten < 0) {
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        if (status == FirstTry) {
          toWrite.insert(std::make_pair(
              fd, WriteEntry(std::move(deferred), buffer.detach(totalWritten),
                             flags)));
        }
    ...
    if (res == -1) {
      ...
      if (res != sizeof(numWakeups)) {
        entry.deferred.reject(
            Net::Error("Read invalid number of bytes for timer fd: " +
