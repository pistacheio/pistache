...
Contact::Contact(std::string ct_name, std::string ct_url, std::string ct_email)
    : name(std::move(ct_name))
    , url(std::move(ct_url))
    , email(std::move(ct_email))
{ }

License::License(std::string lname, std::string lurl)
    : name(std::move(lname))
    , url(std::move(lurl))
{ }

Info::Info(std::string ititle, std::string iversion, std::string idescription)
    : title(std::move(ititle))
    , version(std::move(iversion))
    , description(std::move(idescription))
    , termsOfService()
    , contact()
    , license()
{ }

PathDecl::PathDecl(std::string pvalue, Http::Method pmethod)
    : value(std::move(pvalue))
    , method(pmethod)
{ }

Path::Path(std::string pvalue, Http::Method pmethod, std::string pdescription)
    : value(std::move(pvalue))
    , method(pmethod)
...
PathGroup::const_iterator PathGroup::begin() const {
    return groups_.begin();
}

PathGroup::const_iterator PathGroup::end() const {
    return groups_.end();
}
...
SubPath::SubPath(std::string sprefix, PathGroup* spaths)
    : prefix(std::move(sprefix))
    , parameters()
    , paths(spaths)
{ }
...
SubPath SubPath::path(const std::string& path_prefix) const {
    return SubPath(this->prefix + path_prefix, paths);
}

Parameter::Parameter(std::string pname, std::string pdescription)
    : name(std::move(pname))
    , description(std::move(pdescription))
    , required(true)
    , type()
...
Response::Response(Http::Code rstatusCode, std::string rdescription)
    : statusCode(rstatusCode)
    , description(std::move(rdescription))
{ }
...
void Swagger::install(Rest::Router& router) {
    ...
    Http::ResponseWriter response) {
        ...
        struct Path {
            explicit Path(const std::string& pvalue)
                : value(pvalue)
                , trailingSlashValue(pvalue) {
            ...
            std::string join(const std::string& jvalue) const {
                std::string val;
                if (jvalue[0] == '/')
                    val = jvalue.substr(1);
                else
