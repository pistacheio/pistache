...
void TransportImpl::closePeer(std::shared_ptr<Tcp::Peer>& peer) {
    ...
    if (peer->isIdle()) {
        ...
        response.send(Http::Code::Request_Timeout).then([this, peer](ssize_t) {
            removePeer(peer);
        }, [this, peer](std::exception_ptr) {
            removePeer(peer);
        });
    }
}
...
void Endpoint::bind(const Address& addr) {
    listener.bind(addr);
}
