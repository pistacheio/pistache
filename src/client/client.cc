...
class Transport : public Aio::Handler {
  ...
  private:
    ...
    struct ConnectionEntry {
        ConnectionEntry(Async::Resolver ce_resolve, Async::Rejection ce_reject,
                        std::shared_ptr<Connection> ce_connection,
                        const struct sockaddr* _addr, socklen_t _addr_len)
            : resolve(std::move(ce_resolve))
            , reject(std::move(ce_reject))
            , connection(ce_connection)
            , addr_len(_addr_len) {
    ...
    struct RequestEntry {
        RequestEntry(Async::Resolver re_resolve, Async::Rejection re_reject,
                     std::shared_ptr<Connection> re_connection,
                     std::shared_ptr<TimerPool::Entry> re_timer, std::string buf)
            : resolve(std::move(re_resolve))
            , reject(std::move(re_reject))
            , connection(re_connection)
            , timer(re_timer)
            , buffer(std::move(buf))
        { }
...
Transport::asyncConnect(std::shared_ptr<Connection> connection,
    ...
    [this, address, addr_len, connection](Async::Resolver& resolve, Async::Rejection& reject) {
...
void Connection::connect(const Address& addr) {
    ...
    for (const addrinfo* i_addr = addrs; i_addr; i_addr = i_addr->ai_next) {
        sfd = ::socket(i_addr->ai_family, i_addr->ai_socktype, i_addr->ai_protocol);
        if (sfd < 0)
            continue;
        ...
        transport_
        ->asyncConnect(shared_from_this(), i_addr->ai_addr, i_addr->ai_addrlen)
        .then(
        [this, sfd]() {
...
bool Connection::hasTransport() const {
...
Async::Promise<Response> Connection::perform(const Http::Request& request,
    ...
    [this, request, onDone](Async::Resolver& resolve, Async::Rejection& reject) {
...
void ConnectionPool::init(size_t maxConnsPerHost,
                          size_t maxRespSize) {
    this->maxConnectionsPerHost = maxConnsPerHost;
    this->maxResponseSize       = maxRespSize;
}
    ...
    if (conn == nullptr) {
        ...
        if (!conn->isConnected()) {
            ...
            auto res                           = conn->asyncPerform(request, [this, weakConn]() {
                auto locked_conn = weakConn.lock();
                if (locked_conn) {
                    pool.releaseConnection(locked_conn);
                    processRequestQueue();
                }
            });
